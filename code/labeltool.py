...
class LabelTool():
    def __init__(self, master):
        ...
        imagenum=tk.IntVar()
        self.idxEntry = tk.Entry(self.ctrPanel, width = 5,textvariable=imagenum)
        imagenum.set(0)        
...
##            return
	...
	#pil_image = imgresize(w, h, w_box, h_box, pil_image)
        ...
        if os.path.exists(self.labelfilename):
		    ...
		    #tx = (10, 20, 30, 40)	
                    ...
                    self.listbox.insert(tk.END, '(%.2f,%.2f)-(%.2f,%.2f)' %(tmp[0], tmp[1], tmp[2], tmp[3]) )
    ...
    def mouseClick(self, event):
        if self.STATE['click'] == 0:
            ...
            self.bboxId = None
            self.listbox.insert(tk.END, '(%.2f, %.2f)-(%.2f, %.2f)' %(x1, y1, x2, y2))
            self.listbox.itemconfig(len(self.bboxIdList) - 1, fg = COLORS[(len(self.bboxIdList) - 1) % len(COLORS)])
